...
from airflow.models import DagRun, TaskInstance, SensorInstance
...
class DummySensor(BaseSensorOperator):
    poke_fields = ('input_field', 'return_value')
    ...
    def get_poke_context(self, context):
        result = {key: getattr(self, key, None) for key in self.poke_fields}
        return result

    def get_execution_context(self, context):
        result = {key: getattr(self, key, None) for key in self.exec_fields}
    ...
    def is_smart_sensor_compatible(self):
        return False if self.on_failure_callback else True
...
class SmartSensorTest(unittest.TestCase):
    ...
    def test_load_sensor_works(self):
        ...
        # Test single sensor
        sensor_works = smart._load_sensor_works()
        self.assertEqual(len(sensor_works), 1)
        self.assertEqual(len(smart.cached_dedup_works), 0)
        self.assertEqual(len(smart.cached_sensor_exceptions), 0)
        ...
        sensor_works = smart._load_sensor_works()
        self.assertEqual(len(sensor_works), 3)
        self.assertEqual(len(smart.cached_dedup_works), 0)
        self.assertEqual(len(smart.cached_sensor_exceptions), 0)
    ...
    def test_execute_single_task_with_dup(self):
        ...
        smart._execute_sensor_work(sensor_works[0])

        self.assertEqual(len(smart.cached_dedup_works), 1)
        self.assertIn(sensor_works[0].cache_key, smart.cached_dedup_works)
        self.assertIn(sensor_works[1].cache_key, smart.cached_dedup_works)
        self.assertNotIn(sensor_works[2].cache_key, smart.cached_dedup_works)
        ...
        smart._execute_sensor_work(sensor_works[1])
        self.assertEqual(len(smart.cached_dedup_works), 1)

        time.sleep(1)
        smart._execute_sensor_work(sensor_works[2])
    ...
    @unittest.skip("TODO-MARK: related to freeze_time, to fix")
    def test_smart_operator_timeout(self):
        ...
        for si in sis:
            if si.task_id == SENSOR_OP + "1":
                self.assertEqual(si.state, State.SENSING)

        date2 = date1 + timedelta(seconds=smart.poke_interval)
        ...
        date3 = date2 + timedelta(seconds=smart.poke_interval)
        ...
        for si in sis:
            if si.task_id == SENSOR_OP + "1":
                self.assertEqual(si.state, State.FAILED)
    ...
    def test_register_in_sensor_service(self):
        ...
        self.assertEqual(si.state, State.SENSING)
        self.assertEqual(si.operator, "DummySensor")
